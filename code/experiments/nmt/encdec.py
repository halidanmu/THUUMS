...
class Decoder(EncoderDecoderBase):
    ...
    def build_new_decoder(self, c, y,
            c_mask=None,
            y_mask=None,
            step_num=None,
            mode=EVALUATION,
    ...
    def build_predictor(self, c):
        return self.build_new_decoder(c,None,mode=Decoder.BEAM_SEARCH)
...
class RNNEncoderDecoder(object):
    ...
    def build(self):
        ...
        self.predictions = self.decoder.build_new_decoder(
    ...
    def create_predictor(self):
        out=self.decoder.build_predictor(self.sampling_c);
        self.output_fn = theano.function([self.sampling_x], outputs=[out])
...
def parse_input(state, word2idx, line, raise_unk=False, idx2word=None, unk_sym=-1, null_sym=-1):
    ...
    seqin = line.split()
    seqlen = len(seqin)
    seq = numpy.zeros(seqlen, dtype='int64')
