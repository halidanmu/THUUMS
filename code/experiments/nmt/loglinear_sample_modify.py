...
class BeamSearch(object):
    ...
    def get_tm_score(self, src_seq, aln_array, next_word):
        if not self.phrase_table:
            return 0.0
    ...
    def search(self, sen, seq, n_samples, ignore_unk=False, minlen=1):
        ...
        for k in range(3 * len(seq)):
            ...
            cand_path_idx = cand_idx / voc_size
            cand_word_idx = cand_idx % voc_size
            cand_costs = flat_next_costs[cand_idx]
            cand_rnn_costs = (-1 * log_probs).flatten()[cand_idx]

            #cand_fea_scores: [rnn_costs, lm_costs, tm_costs, unk_nums]
            cand_fea_scores = []
            ...
            for i, (path_idx, next_word, next_cost) in enumerate(
                    zip(cand_path_idx, cand_word_idx, cand_costs)):
                best_aln_idx = numpy.argsort(-aln_score_mat[:,path_idx])[:3]
                best_aln_scores = []
                for aln_idx in best_aln_idx:
                    best_aln_scores.append([aln_idx, aln_score_mat[aln_idx, path_idx]])

                lm_score = -self.get_lm_score(str_trans[path_idx] + self.trg_i2w([next_word]))
                tm_score, unk_tm_num, _ = self.get_tm_score(src_seq, best_aln_scores, self.trg_i2w([next_word])[0])
                tm_score = -tm_score

                if next_word == self.unk_id:
                    unk_trans[path_idx] = 'UNK'
                    #_unk_score = tm_score

                    for t in uni_trans_set:
                        _ls = -self.get_lm_score(str_trans[path_idx] + [t])
                        _ts, _unk_tm_num, match_idx = self.get_tm_score(src_seq, best_aln_scores, t)
                        _ts = -_ts
                        if match_idx == 0 and _ls * self.weight_lm + _ts * self.weight_tm < lm_score * self.weight_lm + tm_score * self.weight_tm:
                            ...
                            unk_trans[path_idx] = t

                cand_costs[i] += lm_score * self.weight_lm + tm_score * self.weight_tm + self.weight_wp
                cand_fea_scores.append([cand_rnn_costs[i], lm_score, tm_score, unk_tm_num])
            ...
            _idx = argpartition(cand_costs.flatten(), n_samples)[:n_samples]
            best_idx = cand_idx[_idx]

            new_trans = [[]] * n_samples
            new_str_trans = [[]] * n_samples
            new_costs = numpy.zeros(n_samples)
            new_fea_scores = [[]] * n_samples
            ...
            for i, (path_idx, next_word, next_cost, _fea_score) in enumerate(
                    zip(cand_path_idx[best_idx], cand_word_idx[best_idx], cand_costs[best_idx], cand_fea_scores[best_idx])):
                new_trans[i] = trans[path_idx] + [next_word]
                if next_word == self.unk_id:
                    new_str_trans[i] = str_trans[path_idx] + [unk_trans[path_idx]]
                else:
                    new_str_trans[i] = str_trans[path_idx] + self.trg_i2w([next_word])
                new_costs[i] = next_cost
                new_fea_scores[i] = fea_scores[path_idx] + [_fea_score]
                #new_lm_costs[i] = lm_costs[path_idx] + [_lm_score]
                #new_tm_costs[i] = tm_costs[path_idx] + [_tm_score]
                #new_unk_nums[i] = unk_nums[path_idx] + [_unk_num]
                #new_rnn_costs[i] = rnn_costs[path_idx] + [_rnn_cost]
                new_aligns[i] = aligns[path_idx] + [aln_score_mat[:,path_idx]]
                for level in range(num_levels):
                    new_states[level][i] = states[level][path_idx]
                inputs[i] = next_word
            ...
            for i in range(n_samples):
                if new_trans[i][-1] != self.enc_dec.state['null_sym_target']:
                    ...
                    fea_scores.append(new_fea_scores[i])
        ...
        if not len(fin_trans):
            if ignore_unk:
                ...
                fin_costs = costs
                fin_aligns = aligns
                fin_fea_scores = fea_scores
                #fin_lm_costs = lm_costs
                #fin_tm_costs = tm_costs
                #fin_rnn_costs = rnn_costs
                #fin_unk_nums = unk_nums

        sorted_idx = numpy.argsort(fin_costs)
        fin_trans = numpy.array(fin_trans)[sorted_idx]
        fin_aligns = numpy.array(fin_aligns)[sorted_idx]
        fin_str_trans = numpy.array(fin_str_trans)[sorted_idx]
        fin_fea_scores = numpy.array(fin_fea_scores)[sorted_idx]
        ...
        return fin_trans, fin_costs, fin_aligns, fin_fea_scores[:, 1], fin_fea_scores[:, 2], fin_str_trans, fin_fea_scores[:, 3], fin_fea_scores[:, 0]
...
def main():
    ...
    for i, line in enumerate(fsrc):
        ...
        trans, costs, trans_ids, aligns, lm_costs, tm_costs, unk_nums, rnn_costs = sample(lm_model, line, seq, n_samples,
